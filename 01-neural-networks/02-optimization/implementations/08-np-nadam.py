...
class NN:
    ...
    def _backward(self):
        for layer in reversed(self.__layers_idxs):
            if layer == self.__layers_idxs[-1]:
                ...
                self.l_grad_weights[layer-1] = self.grad_weights[layer - 1] + (self.beta_1 * self.v_grad_weights[layer-1])  # lookahead
    ...
    def _update_params(self):
        for layer in reversed(self.__layers_idxs):
            self.weights[layer - 1] -= (self.alpha / np.sqrt(self.s_grad_weights[layer - 1]) + self.eps) * self.l_grad_weights[layer - 1]
            self.bias[layer - 1] -= (self.alpha / np.sqrt(self.s_grad_bias[layer - 1]) + self.eps) * self.l_grad_bias[layer - 1]
    ...
    def _init_internals(self):
        ...
        self.l_grad_weights = [0 * l for l in self.__layers_idxs]
        self.l_grad_bias = [0 * l for l in self.__layers_idxs]
...
if __name__ == "__main__":
    ...
    save_params = 'models/nadamNN.pkl'
    load_params = 'models/nadamNN.pkl'
