...
class NN:
    ...
    def train(self, X_train, Y_train, nn_capacity:dict, alpha = .01, gamma = .9, epochs = 250, Y_onehot = None, load_params = False, return_params = True, save_params = False):
    ...
    def _update_params(self):
        for layer in reversed(self.__layers_idxs):
            self.weights[layer - 1] -= (self.alpha / np.sqrt(self.s_grad_weights[layer - 1] + self.eps)) * self.grad_weights[layer - 1]
            self.bias[layer - 1] -= (self.alpha / np.sqrt(self.s_grad_bias[layer - 1] + self.eps)) * self.grad_bias[layer - 1]
...
if __name__ == "__main__":
    ...
    save_params = 'models/rmspropNN.pkl'
    load_params = 'models/rmspropNN.pkl'
    ...
    nnet.train(X_train, Y_train, epochs = epochs, alpha = alpha, gamma = gamma, nn_capacity=nn_capacity, save_params = save_params, load_params = load_params)
