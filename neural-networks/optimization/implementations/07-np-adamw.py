...
class NN:
    ...
    def train(self, X_train, Y_train, nn_capacity:dict, alpha = .01, beta_1 = .99, beta_2 = .99, lambd = .001, epochs = 250, Y_onehot = None, load_params = False, return_params = True, save_params = False):
    ...
    def _update_params(self):
        for layer in reversed(self.__layers_idxs):
            self.weights[layer - 1] -= self.alpha * ((self.v_grad_weights[layer - 1] / np.sqrt(self.s_grad_weights[layer - 1] + self.eps)) + (self.lambd * self.weights[layer - 1]))
...
if __name__ == "__main__":
    ...
    save_params = 'models/adamwNN.pkl'
    load_params = 'models/adamwNN.pkl'
    ...
    nnet.train(X_train, Y_train, epochs = epochs, alpha = alpha, beta_1 = beta_1, beta_2 = beta_2, lambd = lambd, nn_capacity=nn_capacity, save_params = save_params, load_params = load_params)
